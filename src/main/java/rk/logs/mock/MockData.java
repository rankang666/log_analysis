...
//        IP.load("/opt/data/ip/17monipdb/17monipdb.dat");
        //web
        appid.put(0,1000);
        //android
        appid.put(1,1001);
...
//                String[] find = IP.find("118.28.8.8");
                accessLogger.info(String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s"
                        ...
                        , user_agent.get(r.nextInt(user_agent.size()))
        ...
        public void run() {
            ...
            while (true) {
                ...
                headLogger.info(String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s"
                        ...
                        , seid
                        , userid.get(r.nextInt(userid.size()))
                        , param.get(r.nextInt(param.size())), new Date().getTime()));
                //appid	ip	mid	seid	userid	param	result	time
                tailLogger.info(String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s"
                        , appid.get(r.nextInt(appid.size()))
                        , ip.get(r.nextInt(ip.size()))
                        , mid.get(r.nextInt(mid.size()))
                        , seid
                        , userid.get(r.nextInt(userid.size()))
                        , param.get(r.nextInt(param.size()))
                        , new Date().getTime()));
            }
        }
    }
